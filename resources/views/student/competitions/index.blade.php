    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <a href="{{ route('student.competitions.index', ['status' => 'pending']) }}"
                    class="btn btn-sm {{ request('status') == 'pending' ? 'btn-primary' : 'btn-outline-primary' }}">
                    <i class="bi bi-clock"></i> Pending
                </a>
                <a href="{{ route('student.competitions.index', ['status' => 'approved']) }}"
                    class="btn btn-sm {{ request('status') == 'approved' ? 'btn-primary' : 'btn-outline-primary' }}">
                    <i class="bi bi-check-circle"></i> Approved
                </a>
                <a href="{{ route('student.competitions.index', ['status' => 'rejected']) }}"
            ...
            <div class="modal fade" id="addCompetitionModal" tabindex="-1" aria-labelledby="addCompetitionLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-scrollable">
                    <form action="{{ route('student.competitions.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($item->request_verified === 'pending')
                                    <a href="{{ route('student.competitions.edit', $item->competition->competition_id) }}" class="btn btn-warning btn-sm">
                                        <i class="bi bi-pencil-square"></i> Edit
                                    </a>

                                    <form action="{{ route('student.competitions.destroy', $item->competition->competition_id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this request?');">
                                        @csrf
                                        @method('DELETE')
